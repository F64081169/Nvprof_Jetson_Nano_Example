...
def nms_worker(class_id, detections, nms_threshold):
    idxs = np.where(detections[:, 5] == class_id)
    cls_detections = detections[idxs]
    keep = _nms_boxes(cls_detections, nms_threshold)
    return cls_detections[keep]

def parallel_nms(detections, nms_threshold, num_jobs=-1):
    if num_jobs == -1:
        num_jobs = joblib.cpu_count()

    unique_class_ids = list(set(detections[:, 5]))

    nms_detections = joblib.Parallel(n_jobs=num_jobs)(
        joblib.delayed(nms_worker)(class_id, detections, nms_threshold)
        for class_id in unique_class_ids
    )

    nms_detections = np.concatenate(nms_detections, axis=0)
...
def _postprocess_yolo(trt_outputs, img_w, img_h, conf_th, nms_threshold,
    ...
    if len(detections) == 0:
        ...
        nms_detections = parallel_nms(detections, nms_threshold)
