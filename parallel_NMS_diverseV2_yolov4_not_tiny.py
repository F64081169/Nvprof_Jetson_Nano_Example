...
def _nms_boxes(detections, nms_threshold):
    ...
    def process_index(i):
        xx1 = np.maximum(x_coord[i], x_coord[ordered[i + 1:]])
        yy1 = np.maximum(y_coord[i], y_coord[ordered[i + 1:]])
        xx2 = np.minimum(x_coord[i] + width[i], x_coord[ordered[i + 1:]] + width[ordered[i + 1:]])
        yy2 = np.minimum(y_coord[i] + height[i], y_coord[ordered[i + 1:]] + height[ordered[i + 1:]])
