...
class TrtYOLO(object):
    ...
    def detect(self, img, conf_th=0.3, letter_box=None):
        ...
        piece_width = img_resized.shape[1] // 2
        piece_height = img_resized.shape[0] // 2

        piece1 = img_resized[0:piece_height, 0:piece_width]
        piece2 = img_resized[0:piece_height, piece_width:img_resized.shape[1]]
        piece3 = img_resized[piece_height:img_resized.shape[0], 0:piece_width]
        piece4 = img_resized[piece_height:img_resized.shape[0], piece_width:img_resized.shape[1]]

        img_list = [piece1,piece2,piece3,piece4]
        b = []
        s = []
        c = []
        for im in img_list:
            self.inputs[0].host = np.ascontiguousarray(im)
            if self.cuda_ctx:
                self.cuda_ctx.push()
        ...
        b = np.array(b)
        b[:, [0, 2]] = np.clip(b[:, [0, 2]], 0, img.shape[1]-1)
        b[:, [0, 2]] = np.clip(b[:, [0, 2]], 0, img.shape[1]-1)
        return b,s,c
